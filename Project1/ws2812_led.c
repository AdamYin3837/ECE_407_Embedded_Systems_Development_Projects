...
void ws2812_init(void) {
    ...
    ws2812_program_init(ws2812_pio, ws2812_sm, ws2812_offset, WS2812_PIN, WS2812_FREQ, WS2812_IS_RGBW);
}
...
void ws2812_update_angle(int angle_degrees) {
    ...
    uint32_t on_color = urgb_u32(0, 32, 0); // Green at low brightness

    for (int i = 0; i < WS2812_NUM_LEDS; ++i) {
        if (i == index) {
            ws2812_put_pixel(on_color);
        } else {
            ws2812_put_pixel(0);
        }
    }
